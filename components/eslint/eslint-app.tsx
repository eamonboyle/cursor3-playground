...
import { SAMPLE_ESLINT_OUTPUT } from "@/lib/eslint/defaults"
import {
  diagnosticLocation,
  formatEslintMarkdown,
  formatEslintPaths,
  parseEslintOutput,
} from "@/lib/eslint/parse"
import type { EslintDiagnostic } from "@/lib/eslint/types"
    ...
    <TableRow>
      <TableCell className="font-mono text-xs">
        <Badge
          ...
          className="mr-2 max-w-[8rem] truncate"
          title={diagnostic.rule || undefined}
        >
          {diagnostic.rule || "—"}
        </Badge>
      </TableCell>
...
export function EslintApp() {
  const [input, setInput] = React.useState(SAMPLE_ESLINT_OUTPUT)
  const [hideNodeModules, setHideNodeModules] = React.useState(true)

  const result = React.useMemo(
    () => parseEslintOutput(input, { hideNodeModules }),
  ...
  const topRules = Object.entries(result.summary.byRule)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 6)
  ...
  return (
    <div className="mx-auto flex min-h-svh max-w-3xl flex-col gap-8 p-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">
              ESLint diagnostic lab
            </h1>
            <p className="text-muted-foreground text-sm">
              Paste <code className="text-xs">eslint</code> or{" "}
              <code className="text-xs">pnpm lint</code> output — group by rule,
              filter node_modules, and copy file:line paths for Cursor.
            </p>
          </div>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Linter output</CardTitle>
          <CardDescription>
            Supports ESLint stylish (default), unix, and compact formats,
            including ANSI-colored CI logs.
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col gap-4">
          <Field>
            <FieldLabel htmlFor="eslint-output">Lint log</FieldLabel>
            <FieldContent>
              <Textarea
                id="eslint-output"
                value={input}
                ...
                spellCheck={false}
                placeholder="Paste eslint output here"
              />
            </FieldContent>
          </Field>
          <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              <Button
                ...
                size="sm"
                onClick={() => {
                  setInput(SAMPLE_ESLINT_OUTPUT)
                  toast.message("Loaded sample lint output.")
                }}
              >
                Load sample
              </Button>
      ...
      <Card>
        ...
        <CardContent>
          {result.diagnostics.length === 0 ? (
            <p className="text-muted-foreground text-sm">
              No diagnostics yet. Paste lint output or load the sample.
            </p>
          ) : (
      ...
      <p className="text-muted-foreground text-center text-xs">
        Tip: run <code className="rounded bg-muted px-1">pnpm lint</code>{" "}
